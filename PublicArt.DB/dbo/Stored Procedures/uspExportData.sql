        ...
        [CTE_ItemImageFlat]([ItemId], [ImageFileNames], [ImageCaptions])
          AS (SELECT
              ...
              , [ImageFileNames] = STUFF((SELECT
                                            '|' +
                                            CONVERT(VARCHAR(40), [it1].[Reference]) + '-' + SUBSTRING(N'abcdefghijklmnopqrstuvwxyz',ROW_NUMBER() OVER (Order by [i2].[Primary] DESC, [i2].[stream_id]),1) +
                                            '.jpg'
                                          FROM
                                            [dbo].[ItemImage] [i2]
										  JOIN [dbo].[Item] [it1]
											ON [i1].[ItemId] = [it1].[ItemId]
                                          WHERE
                                            [i2].[ItemId] = [i1].[ItemId]
                                          ORDER BY
